...
const degreeLevels = ["Junior", "Junior+", "Middle", "Middle+", "Senior"];

const DegreeDropdown = ({ onChange }) => {
  const [selected, setSelected] = useState(degreeLevels[0]);
            ...
            >
              <span
                className={`w-4 h-4 flex items-center justify-center border rounded-full ${
                  selected === level ? "border-blue-500 bg-blue-500" : "border-gray-400"
                }`}
              >
                {selected === level && (
